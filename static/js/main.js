...
function changeCSSStyle (selector, cssProp, cssVal) {
    ...
    for (i = 0, len = document.styleSheets[mIndex][cssRules].length; i < len; i++) {
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (a?.length === 3) {
        ...
        document
            ...
            .forEach(e => new bootstrap.Tooltip(e, { title: amountTooltip, html: true }));
    }

    document
        .querySelectorAll('[alias-type]')
        .forEach(e => new bootstrap.Tooltip(e, { title: addressAliasTooltip, html: true }));
    ...
    document.querySelectorAll('.copyable').forEach(e =>
        e.addEventListener('click', e => {
            if (e.clientX < e.target.getBoundingClientRect().x) {
                ...
                setTimeout(
                    () => (e.target.className = e.target.className.replace('copied', 'copyable')),
                    1000,
                );
                e.preventDefault();
            }
