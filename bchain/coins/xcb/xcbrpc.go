...
func (b *EthereumRPC) getBlockRaw(hash string, height uint32, fullTxs bool) (json.RawMessage, error) {
	...
	if hash != "" {
		if hash == "pending" {
			...
			err = b.rpc.CallContext(ctx, &raw, "xcb_getBlockByHash", ethcommon.HexToHash(hash), fullTxs)
		}
	} else {
...
func (b *EthereumRPC) getxrc20EventsForBlock(blockNumber string) (map[string][]*rpcLog, error) {
	ctx, cancel := context.WithTimeout(context.Background(), b.timeout)
	defer cancel()
